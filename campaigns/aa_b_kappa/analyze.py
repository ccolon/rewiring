...
COL_TITLES = {"DRS": r"DRS ($b_i = 0.9$)",
              "CRS": r"CRS ($b_i = 1.0$)",
              "IRS": r"IRS ($b_i = 1.1$)",
              "MIX": r"HRS ($b_i \sim U[0.9, 1.1]$)"}
...
def render_figure(bundle, out_path_prefix, left_metric="rewirings"):
    ...
    left_color = "C0"   # blue series colour for the left-axis tinting
    ...
    for cname, agg in cells.items():
        ...
        if col == 0:
            ...
            # Left axis (label + ticks + spine) in blue to match the C0 series.
            # Use the default labelpad so the y-label sits close to the axis
            # and the kappa marker (placed further left) doesn't overlap it.
            ax.set_ylabel(left_ylabel, color=left_color, fontsize=9)
            ax.tick_params(axis="y", colors=left_color, labelcolor=left_color)
            ax.spines["left"].set_color(left_color)
            # kappa row marker -- stays black, well to the left of the y-label.
            ax.text(-0.32, 0.5, rf"$\kappa = {kappa_val}$",
                    transform=ax.transAxes, rotation=90,
                    ha="center", va="center", color="black", fontsize=11)
        if row == 1:
            ax.set_xlabel(r"Round $q$")
        if col == 3:
            ax2.set_ylabel(r"Cross-seed distance $\bar{d}_q$", color="C3",
                           fontsize=9)
            ax2.tick_params(axis="y", colors="C3")

    # Three per-axes legends (no figure-level legend).
    # Bottom-left (row 1, col 0): the two blue-series entries.
    bl_handles = [
        plt.Line2D([0], [0], color="C0", lw=1.6, label=left_legend),
        plt.Line2D([0], [0], color="C0", lw=0.5, alpha=0.5,
                   label=left_seed_legend),
    ]
    axes[1, 0].legend(handles=bl_handles, loc="upper right",
                      fontsize=8, framealpha=0.95)

    # Top-right of the inner CRS column (row 0, col 1): median convergence.
    tr_handles = [
        plt.Line2D([0], [0], color="grey", ls=":", lw=0.8,
                   label="median round\nfor convergence"),
    ]
    axes[0, 1].legend(handles=tr_handles, loc="upper right",
                      fontsize=8, framealpha=0.95)

    # Bottom of the inner CRS column (row 1, col 1): red dispersion entries.
    br_handles = [
        plt.Line2D([0], [0], color="C3", lw=1.6, ls="--",
                   label=r"$\bar{d}_q$"),
        plt.Rectangle((0, 0), 1, 1, fc="C3", alpha=0.18,
                      label="10-90% band"),
    ]
    axes[1, 1].legend(handles=br_handles, loc="upper right",
                      fontsize=8, framealpha=0.95)
...
# =============================================================================
def main():
    # Tracked code in campaigns/aa_b_kappa/; raw sweep data + figures live in
    # the mirroring results/aa_b_kappa/ (gitignored).
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    REPO_ROOT = os.path.dirname(os.path.dirname(SCRIPT_DIR))
    RESULTS_DIR = os.path.join(REPO_ROOT, 'results', 'aa_b_kappa')
    DEFAULT_SWEEP_DIR = os.path.join(RESULTS_DIR, 'b_ms1-4')
    DEFAULT_FIG_DIR = os.path.join(RESULTS_DIR, 'figures')
